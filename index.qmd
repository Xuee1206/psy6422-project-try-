...
The animated figure (Plot 1a) shows how happiness changes year by year in seven European countries. Because the lines appear slowly over time, it is easier to notice when each country goes up or down. For example, in some years the line suddenly rises or drops, and the animation helps us see exactly when that happens. This makes the short-term changes more obvious, which is harder to notice in a static graph.

The static figure (Plot 1b) gives a clearer overview of the long-term pattern. I chose seven well-known countries so the lines would not be too crowded. From the plot we can see that the trends do not follow one simple direction. Instead, most countries move up and down across the years. For example, Sweden stays at a very high level throughout almost the whole period, while Spain and Ireland start lower but show some noticeable increases later on. Germany and the United Kingdom also have more obvious fluctuations, especially around the 1980s and 1990s.

The static graph also includes a LOESS smooth line (the dashed black line). This line shows the average trend across all seven countries. It makes the general direction easier to understand because it removes some of the noise caused by year-to-year changes. The smooth line suggests that overall happiness in Europe stayed quite stable over the four decades, with a small increase after the early 2000s.

Together, the two plots help answer the first research question.\
The animated plot helps us see **when and how** happiness changed each year, while the static plot helps us understand **the long-term levels and overall pattern**. Although individual countries have their own ups and downs, the general trend across Europe remains fairly steady.
